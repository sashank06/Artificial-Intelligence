...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            actionNode = PgNode_a(action) #create a node for the action
            #check if the actionNode satisfies the precondtion of the previous level
            if actionNode.prenodes.issubset(self.s_levels[level]):
                action_nodes.append(actionNode) # add the node to the empty list

                #create the children for the previous node and add the previous node as parent to the current one.
                for node in self.s_levels[level]:
                    node.children.add(actionNode)
                    actionNode.parents.add(node)
    ...
    def add_literal_level(self, level):
        ...
        nodes_s =set() # we use a set to avoid duplication
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #effect of one action is the negation of the precondition of the other
        #case  1
        for eff in node_a1.action.effect_add:
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            ...
            for level in range(len(self.s_levels)):
                for node in self.s_levels[level]:
                    if expr(node.symbol) == goal and node.is_pos:
                        goal_achieved= True
                        level_sum += level
                        break
                if goal_achieved:
